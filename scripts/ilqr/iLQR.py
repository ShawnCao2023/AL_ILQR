...
class iLQR():
    def __init__(self, args, obstacle_bb, polylines, verbose=False):
        self.args = args
    ...
    def backward_pass(self, X, U, poly_coeff, x_local_plan, npc_traj, lamb):
        ...
        #self.constraints.check_boundaries_constrains()

        l_x, l_xx, l_u, l_uu, l_ux = self.constraints.get_cost_derivatives(X[:, 1:], U, poly_coeff, x_local_plan, npc_traj)
        lagrange_cost_d, lagrange_cost_dd = self.constraints.get_boundaries_cost(X[:, 1:])
        ...
        for i in range(self.args.horizon-1,-1,-1):
            Q_x = l_x[:,i] + df_dx[:,:,i].T @ V_x + lagrange_cost_d[:,i]
    ...
    def run_step(self, ego_state, npc_traj):
        ...
        X, U = self.get_optimal_control_seq(X_0, self.control_seq, poly_coeff, ref_traj, npc_traj)
        traj = X[:2, ::int(self.args.horizon/10)].T

        self.control_seq = U
        self.plot(U, X, ref_traj, npc_traj)
    ...
    def get_optimal_control_seq(self, X_0, U, poly_coeff, x_local_plan, npc_traj):
        ...
        for itr in range(self.args.max_iters):
            k, K = self.backward_pass(X, U, poly_coeff, x_local_plan[:,0], npc_traj, lamb)
            print ("Iteration: ", itr)
            self.plot(U, X, x_local_plan, npc_traj)
            time.sleep(0.5)
            # Get control values at control points and new states again by a forward rollout
            X_new, U_new = self.forward_pass(X, U, k, K)
            J_new = self.constraints.get_total_cost(X, U, poly_coeff, x_local_plan[:,0], npc_traj)
    ...
    def plot(self, control, X, ref_traj, npc_traj):
        self.ax1.clear()
        ...
        for i in range(npc_traj.shape[1]):
            xy = (npc_traj[0,i]-npc_x/2, npc_traj[1,i]-npc_y/2)
            rc = patches.Rectangle(xy, npc_x,npc_y, fill=False)
            self.ax2.add_patch(rc)
